...
object BehaviourProperties:
   ...
   */
  sealed trait PropertyCheck[T]:
    def isValid(traces: Iterable[T]): Boolean

  private trait MultiSetProperty[T] extends PropertyCheck[MSet[T]]
  ...
  private case class Reachability[T](private val state: T) extends MultiSetProperty[T]:
    override def isValid(traces: Iterable[MSet[T]]): Boolean = traces.exists(multiSet => multiSet(state) > 0)

  /** Deadlock-freeness
   * Definition: Checks that the system never reaches a marking where no transition is enabled (no further progress possible).
   */
  private case class DeadlockFreeness[T](private val deadlockCondition: T => Iterable[T]) extends PropertyCheck[T]:
    override def isValid(traces: Iterable[T]): Boolean =
  ...
  private case class Fairness[T](private val allowedStates: Set[T]) extends MultiSetProperty[T]:
    override def isValid(traces: Iterable[MSet[T]]): Boolean =
      val frequencies: Map[T, Int] = traces
        .map(mset => mset.asMap)
        .flatMap(map => map.toList)
        .groupMapReduce(l => l._1)(l => l._2)((f1, f2) => f1 + f2)

      allowedStates.forall(state => frequencies.getOrElse(state, 0) > 0)
  ...
  def reachability[T](state: T): PropertyCheck[MSet[T]] = Reachability[T](state)
  def deadlockFreeness[T](deadlockCondition: T => Iterable[T]): PropertyCheck[T] = DeadlockFreeness[T](deadlockCondition)
  def fairness[T](allowedStates: Set[T]): PropertyCheck[MSet[T]] = Fairness[T](allowedStates)
  def reversibility[T](initialState: T): PropertyCheck[MSet[T]] = Reversibility[T](initialState)
